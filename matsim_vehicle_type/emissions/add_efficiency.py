...
def add_efficiency(
    emission_df: pd.DataFrame, efficiency_df: pd.DataFrame
) -> pd.DataFrame:
    """
    ...
    efficiency_df = efficiency_df.loc[
        efficiency_df["AnneeSAAQ"] == SCN_YEAR, ["predicted_improvement_pct", "fsa"]
    ]
    emission_df = emission_df.merge(
        efficiency_df, left_on="CFSAUID", right_on="fsa", how="left"
    )
