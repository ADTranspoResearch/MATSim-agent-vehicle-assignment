...
def build_external_annual_benchmark(path: Path = EXTERNAL_FILE) -> pd.DataFrame:
    ...
    quarterly_cube = external_long.pivot_table(
        index=["year", "quarter"],
    ...
    counts["hev_sedan"] = qcol("Passenger cars", "Plug-in hybrid electric") + qcol(
        "Passenger cars", "Hybrid electric"
    )
    counts["hev_suv"] = qcol(
        "Multi-purpose vehicles", "Plug-in hybrid electric"
    ...
    counts["total_registrations"] = quarterly_cube.xs(
        "Total, vehicle type", axis=1, level=0
    ).sum(axis=1)
    counts["ice_van/pickup"] = (
        counts["total_registrations"]
        - counts[["electric", "hev_sedan", "hev_suv", "ice_sedan", "ice_suv"]].sum(
            axis=1
        )
    ).clip(lower=0)

    counts = counts.reset_index()
    counts["quarter_num"] = counts["quarter"].map(
        {quarter: idx + 1 for idx, quarter in enumerate(QUARTER_ORDER)}
    )
    ...
    annual_counts = counts.groupby("year")[
        VEHICLE_TYPES + ["total_registrations"]
    ].sum()
    annual = annual_counts[VEHICLE_TYPES].div(
        annual_counts["total_registrations"], axis=0
    )

    row_2025 = trailing.loc[
        (trailing["year"] == 2025) & (trailing["quarter"] == "T1")
    ].iloc[0]
    for col in VEHICLE_TYPES:
        annual.loc[2025, col] = (
            row_2025[f"{col}_trailing4"] / row_2025["total_registrations_trailing4"]
        )
...
def load_population_q1_series(path: Path = POPULATION_FILE) -> pd.Series:
    ...
    population_quebec = population_quebec.drop(
        columns=[
            col for col in population_quebec.columns if str(col).startswith("Unnamed")
        ],
        errors="ignore",
    )
    pop_row = population_quebec.iloc[0].drop(labels=["Geography"])
    pop_series = pd.to_numeric(
        pop_row.astype(str).str.replace(",", "", regex=False), errors="coerce"
    ).dropna()
